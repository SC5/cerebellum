  ...
  Object.keys(routes).forEach(function(route) {
    app.get(route, function(req, res) {
      ...
      Promise.resolve(context).then(function(context) {
        ...
        return Promise.resolve(routes[route].apply(context, utils.extractParams(route, req.params))).then(function(options) {
          var document = cheerio.load(indexHTML);
          if (context.store) {
            storeServerState( document, context.store.export() );
          }
